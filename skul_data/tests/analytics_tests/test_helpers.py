...
def create_test_action_log(
    user, category, timestamp=None, content_type=None, object_id=None, **kwargs
):
    """Helper to create an action log entry with all required fields"""
    from skul_data.action_logs.models.action_log import ActionLog, ActionCategory

    # Validate category
    if category not in ActionCategory.values:
        raise ValueError(f"Invalid category. Must be one of: {ActionCategory.values}")

    # Set default action based on category
    action = kwargs.pop("action", f"Test {category.lower()} action")
    ...
    return ActionLog.objects.create(
        ...
        user_agent=kwargs.pop("user_agent", "TestAgent/1.0"),
